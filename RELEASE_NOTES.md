...
* [Adding] FPGA place-and-route for Lattice ECP5 and Gowin next to the existing Lattice iCE40 flow. `nextpnr` is now built for `ice40`, `ecp5` and `himbaechel` in one pass, with [`prjtrellis`](https://github.com/YosysHQ/prjtrellis) supplying `ecppack` and the ECP5 database and [`apycula`](https://pypi.org/project/apycula) supplying `gowin_pack` and the Gowin database (all Gowin devices, not only the ones on the popular boards). Yosys already carried every `synth_*` pass, so only the back ends were missing.
* [Adding] the [openXC7](https://github.com/openXC7/nextpnr-xilinx) toolchain for Xilinx 7-series FPGAs as the new `fpga-xilinx` tool image: `nextpnr-xilinx` together with [`prjxray`](https://github.com/f4pga/prjxray), which brings `fasm2frames`, `xc7frames2bit` and the bitstream database. Upstream ships this only as a nix flake, so designs targeting Artix-7 or Spartan-7 boards had to leave the container for it. `NEXTPNR_XILINX_PYTHON_DIR` and `PRJXRAY_DB_DIR` are exported, and the chipdb of a part is generated once in the design directory with `bbaexport.py` and `bbasm`, which is why it is not baked into the image. The prjxray database ships the artix7, kintex7, spartan7 and zynq7 families, selectable at build time through the `FPGA_XILINX_FAMILIES` argument (virtex7 is dropped, the flow cannot map its parts).
* [Adding] test 32, which synthesizes the same blinky for all four FPGA families and takes iCE40, ECP5 and Gowin through to a bitstream.
