...
# so that relative symlinks in sg13cmos5l that point to sg13g2 files remain valid)
echo "[INFO] Cloning IHP SG13CMOS5L PDK."
git clone https://github.com/IHP-GmbH/ihp-sg13cmos5l.git
SG13CMOS5L_COMMIT=$(git -C ihp-sg13cmos5l rev-parse HEAD)
...
# Install IHP-SG13CMOS5L additional setup
# Note: osdi/xyce symlinks in sg13cmos5l use relative paths pointing to ihp-sg13g2,
# so they resolve correctly now that both PDKs are siblings in $PDK_ROOT.
PDK="$PDK_CMOS5L"
